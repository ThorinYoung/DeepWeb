...
state_dict = torch.load('remaster/model/remasternet.pth.tar')
...
outputdir_out = outputdir + 'repair_res/'
...
with torch.no_grad():
    ...
    num = 0
    while True:
        with open("remaster/process.cfg", "r+")as f:
            sign = f.read()
            if sign == "stop":
                f.write("no")
                f.close()
        ...
        num += proc_g
        pbar.update(proc_g)
        print("+" + outputdir_out + '%07d.png' % index + '+\n')
        print("||", num / nframes, "||\n")
